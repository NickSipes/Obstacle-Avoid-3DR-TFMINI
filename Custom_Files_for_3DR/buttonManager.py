...
class buttonManager():
    ...
    def handleButtons(self, buttonEvent):
        ...
        if button == btn_msg.ButtonCameraClick and event == btn_msg.LongHold:
            if self.scanactive == 0:
                logger.log("[button]: Turn scanner on")
                try:
                ...
                try:
                    self.shotMgr.arduinoBoard.digital_write(SCAN_ENABLE, 0)
                    self.scanactive = 0
                   # send status to app
                    if self.shotMgr.appMgr.isAppConnected():
